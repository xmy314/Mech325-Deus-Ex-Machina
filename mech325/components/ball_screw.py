...
def retrieve_ballscrew_information():
    ...
    pathways = [
        ...
        (PathType.EQUATION, "General Equation Geometry", Geqn(S("n"), S("V")/S("L")*60)),
        (PathType.EQUATION, "General Equation Power", Geqn(S("P"), S("T_u")*S("n")/63025)),


        (PathType.TABLE_OR_FIGURE, "Thomson_Ball_Screw life", [[S("L_{d\\expected}")], [S("d"), S("l"), S("F")]]),

        (PathType.EQUATION, "General Equation Safety Factor", Geqn(S("F_{des}"), S("N")*S("F"))),
